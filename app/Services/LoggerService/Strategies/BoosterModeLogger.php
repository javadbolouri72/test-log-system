<?php

namespace App\Services\LoggerService\Strategies;

class BoosterModeLogger extends Logger
{

    public static function startLogSession(): void
    {
        // TODO: Implement startLogSession() method.
    }

    public static function externalServiceLog(): void
    {
        // TODO: Implement externalServiceLog() method.
    }

    public static function queryLog(): void
    {
        // TODO: Implement queryLog() method.
    }

    public static function commandLog(): void
    {
        // TODO: Implement commandLog() method.
    }

    public static function exceptionLog(): void
    {
        // TODO: Implement exceptionLog() method.
    }

    public static function finishLogSession(): void
    {
        // TODO: Implement finishLogSession() method.
    }
}
